...
export const setSelectedDatasetProfile = async (profile: string) => {
  await AsyncStorage.setItem(DATASET_PROFILE_KEY, profile);
};

export const getSelectedDatasetProfile = async (): Promise<string | null> => {
  return await AsyncStorage.getItem(DATASET_PROFILE_KEY);
};

export const setSelectedModelProfile = async (profile: string) => {
  await AsyncStorage.setItem(MODEL_PROFILE_KEY, profile);
};

export const getSelectedModelProfile = async (): Promise<string | null> => {
  return await AsyncStorage.getItem(MODEL_PROFILE_KEY);
};

// ─── Dataset Profiles ─────────────────────────────────────────────────────────

export const getDatasetProfiles = async (): Promise<string[]> => {
  const json = await AsyncStorage.getItem(DATASET_PROFILES_LIST_KEY);
  return json ? JSON.parse(json) : ['Apples', 'Oranges', 'Bananas'];
};

export const addDatasetProfile = async (name: string) => {
...
export const removeDatasetProfile = async (name: string) => {
  const profiles = await getDatasetProfiles();
  const filtered = profiles.filter(p => p !== name);
  await AsyncStorage.setItem(DATASET_PROFILES_LIST_KEY, JSON.stringify(filtered));
};
